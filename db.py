...
def listar_imoveis(preco_min=None, preco_max=None, bairros=None, cidades=None, ordenar_por="recentes"):
...
def listar_bairros():
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT DISTINCT bairro FROM imoveis WHERE bairro IS NOT NULL ORDER BY bairro"
        ).fetchall()
